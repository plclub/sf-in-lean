...
questions, inline review threads, TODO items.  Use freely.  It takes two
optional named arguments, so the note's provenance can be typeset uniformly by
a future dev-facing build: `author` (a string, conventionally
`"Full Name (github-handle)"`) and `urgency` (an identifier or string,
conventionally `SOONER`, `LATER`, `TODO`, or `TOFIX`).  Prefer the arguments
over leading `BCP:` / `SOONER:` tags in the body; `to_verso` promotes such
leading tags to arguments automatically (see `_AUTHOR_NAMES` in
`scripts/to_verso.py` for the initials-to-name mapping).

```
:::dev (author := "Benjamin Pierce (bcpierce00)") (urgency := SOONER)
