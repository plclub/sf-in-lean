...
(* SOONER: Seems a bit short for how essential the topics are.

   MRC 3/22: What if this chapter were to be inlined in previous
   chapters?  That way induction wouldn't stay a mystery for so long,
   and this chapter wouldn't have to seem short. The parts about
   [nat_ind] could go in Induction; about lists, in either Lists or
   Poly; about propositions, in IndProp; and about proof objects, in
   ProofObjects. The main hiccup seems to be that [apply] isn't
   introduced until Tactics. *)

(** FULL: Every time we declare a new [Inductive] datatype, Rocq
...
(** FULL: Here is the induction principle for natural numbers: *)

(** TERSE: The automatically generated _induction principle_ for [nat]: *)
...
(** FULL: The [induction] tactic is a straightforward wrapper that, at its
...
(** TERSE: We can directly use the induction principle with [apply]: *)
...
Proof.
  ...
  apply nat_ind.
  - (* n = O *) reflexivity.
  - (* n = S n' *) simpl. intros n' IHn'. rewrite -> IHn'. reflexivity.  Qed.
(* /ADMITTED *)
(** [] *)
(* /FULL *)

(** FULL: Rocq generates induction principles for every datatype
...
(** TERSE: *** *)
(** TERSE: Rocq generates induction principles for every datatype defined with
    [Inductive], including those that aren't recursive. *)

(** FULL: These generated principles follow a similar pattern. If we
...
(** TERSE: If we define type [t] with constructors [c1] ... [cn],
    Rocq generates:
[[
    t_ind : forall P : t -> Prop,
              ... case for c1 ... ->
...
(** FULL: Before trying to write down a general rule, let's look at
...
(** TERSE: *** *)
(** FULL: Here's another example, this time with one of the constructors
...
Definition leaf_case (P : booltree_property_type) : Prop
  (* ADMITDEF *) := forall b : bool, P (bt_leaf b).
(* /ADMITDEF *)
...
(** FULL: This extra naming step isn't something that we do in
...
(** TERSE: The _induction hypothesis_ is the premise of the
    implication [P_m0r n' -> P_m0r (S n')] -- the assumption that [P]
    holds of [n'], which we are allowed to use in proving that [P]
    holds for [S n']. *)
...
Theorem add_assoc_P : forall m p n : nat,
  P_assoc n m p.
Proof.
  intros m p. apply nat_ind.
  - (* n = O *) unfold P_assoc. reflexivity.
  - (* n = S n' *)
...
Theorem add_comm_P : forall m n : nat,
  P_comm n m.
Proof.
  intros m. apply nat_ind.
  - (* n = 0 *) unfold P_comm. rewrite -> add_0_r. reflexivity.
  - (* n = S n' *)
...
(** FULL: This definition can be streamlined a little by observing that the
...
(** TERSE: [n] could instead be a parameter:  *)
(* HIDE: That's something that we should really discuss earlier --
   probably in Ind.v.   APT: But none of the Inductive definitions
   currently in that file are suitable examples. *)
...
(* HIDE: Confirmed: This is exactly the same as Rocq's. *)

Notation "m <=2 n" := (le2 m n) (at level 70).

(* HIDEFROMADVANCED *)
(** FULL: The second one is better, even though it looks less symmetric.
...
(** TERSE: The latter is simpler, and corresponds to Rocq's own
    definition. *)
...
(* SOONER: From Marc Bezem, 2022...
  ...
  I was tempted to say more about the type of P in line 540:

  "forall P : (forall n : nat, ev n -> Prop), ..."

  Pedagogy: Induction is a principle for proving that a given unary
  predicate P holds for all elements of an inductively defined "thing".

  The question here is: what is this "thing" in the case of the inductively
  defined ev: nat -> Prop, that is, ev is a _family_ of propositions, not a set or so.

  One answer is (sigT ev) : Set, the set of pairs (n,E) with n: nat and E: ev n.
  Unfortunately, sigT comes from a part of Rocq that we have not explained.
  It is a generalization of exists from Prop to arbitrary universes (Set, Type).
  The type of P can now be understood as a curried form of (sigT ev) -> Prop,
  according to the following two lemmas.

  Lemma decurryingT : forall (X:Type) (P : X -> Type) (Q : Type),
    (forall x, P x -> Q) -> (sigT (fun x =>  P x) -> Q).
  Proof.
    intros X P Q HallPQ HexP. destruct HexP as [x HPx]. apply (HallPQ x HPx).
  Qed.

  Lemma encurryingT : forall (X:Type) (P : X -> Type) (Q : Type),
    (sigT (fun x =>  P x) -> Q) -> (forall x, P x -> Q).
  Proof.
    intros X P Q HexPQ x HPx. apply HexPQ. exists x. assumption.
  Qed.
*)
...
(* HIDE *)
    ...
    (* SOONER: APT: Not sure where this is supposed to live. *)
...
(* HIDE: APT: Rest of this file is very old stuff I rescued. *)
(* ######################################################### *)
(** * Explicit Proof Objects for Induction (Optional) *)

(* HIDE: APT: I'd like to see this included, but perhaps it is too
   advanced.  And perhaps it should go at the end of the previous
   chapter (but the motivating example works better here. *)
(* HIDE: BCP: I've included it and marked it as "clearly optional". *)
(* HIDE: MRC 3/22: Unless I'm missing something, IndPrinciples could
   come before ProofObjects if it were not for this section. The flow
   might be better that way: IndProp->IndPrinciples->ProofObjects. All
   that would be needed is to move this section into ProofObjects as
   APT mentions. *)
...
(* HIDE *)
(* A quick check that [nat_ind_tidy] works. *)
Theorem mul_0_r''' : forall n:nat,
  n * 0 = 0.
Proof.
  apply nat_ind_tidy.
...
(** FULL: We can read [build_proof] as follows: Suppose we have
...
(** FULL: The actual [nat_ind] that Rocq generates uses a recursive
    function [F] defined with [fix] instead of [Fixpoint]. *)

(* HIDE: (no longer relevant in recent Rocq versions)
    Aside to those interested in functional programming: You may
    notice that the [match] in [F] requires an annotation [as n0
    return (P n0)] to help Rocq's typechecker realize that the two arms
...
(** FULL: We can adapt this approach to proving [nat_ind] to help prove
    ...
    relating the [ev] predicate we defined in \CHAP{IndProp}
    to the [even] function defined in \CHAP{Basics}. *)

(** TERSE: We can also define _non-standard_ induction principles
...
(* HIDE: Actually, proving the lemma with tactics is not very hard if
         we use the fix tactic, but that would be hard to
         explain... *)

(* HIDE: Could use an exercise here. Palindrome_converse is too hard, though!

[[
    rev_ind : forall (X : Type) (P : list X -> Prop),
    P [] -> (forall x, P [x]) ->
    (forall h t g, P t -> P (h :: t ++ [g])) -> forall l, P l
]]

    is an example of a more useful induction principle
    for the following theorem from \CHAP{IndProp}.

[[
    Theorem palindrome_converse : forall (X : Type) (l : list X),
     l = rev l -> pal l.
    Proof.
     intros X. apply (rev_ind X (fun l => l = rev l -> pal l)).
     - intros H. apply pal_nil.
     - intros x H. apply pal_sing.
     - intros h t g H G. assert (K : h = g /\ t = rev t).
       + replace (h :: t ++ [g]) with ((h :: t) ++ [g]) in G.
         * rewrite rev_app_distr in G. inversion G as [[Gh Gt]].
           apply (f_equal _ _ rev) in Gt. replace [g] with (rev [g]) in Gt.
           { rewrite <- rev_app_distr in Gt. rewrite rev_involutive in Gt.
             rewrite rev_app_distr in Gt. inversion Gt as [Gtt]. split.
             - reflexivity.
             - rewrite Gtt, Gtt. reflexivity. }
           { reflexivity. }
         * reflexivity.
       + destruct K as [Kh Kt]. rewrite Kh.
         apply pal_cons. apply H. apply Kt. Qed.
 ]]
*)

(* HIDE: MRC 3/22: Added the following exercise to see whether it works
   for people. *)
...
(* HIDE: MRC 3/22: Unfortunately we have to define syntax for tuples,
   because Poly shadowed the standard library definition of [pair]. *)
...
Definition better_t_tree_ind : better_t_tree_ind_type
  (* ADMITDEF *) :=
  fun X P base_case ind_step =>
    fix build_proof t :=
    match t with
    | t_leaf => base_case
    | t_branch (l, v, r) => ind_step v l (build_proof l) r (build_proof r)
    end.
(* /ADMITDEF *)
...
Theorem reflect_involution : forall (X : Type) (t : t_tree X),
    reflect (reflect t) = t.
Proof. (* ADMITTED *)
  intros X. induction t using (better_t_tree_ind X).
  - reflexivity.
