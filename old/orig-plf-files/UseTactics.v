...
(** Two tactics help in case analysis:
...
(** As you probably have learned, the tactic [destruct] can be used
    ...
    produce the equality [eqb_id k1 k2 = true] rather than the
    equality [true = eqb_id k1 k2]. Indeed, the former reads much more
    ...
    an equality in the form [true = eqb_id k1 k2] rather than
    [eqb_id k1 k2 = true]. The following examples illustrate the
...
Proof.
  ...
  (* The old proof: *)
  remember (eqb_id k1 k2) as b. destruct b.
    apply eqb_id_true in Heqb. subst. reflexivity.
    reflexivity.

  (* The new proof: *)
  cases' (eqb_id k1 k2) as E.
    apply eqb_id_eq in E. subst. reflexivity.
    reflexivity.
Qed.
...
(** The tactics [applys], [forwards] and [specializes] are
    ...
    use of [forwards] can be found in the tutorial chapter \CHAP{UseAuto}. *)
(* HIDE *)
(* NOWISH: This substitution_preserves_typing proof relies on lemmas that
are no longer presented to the students in current versions of the textbook,
so this proof either needs to be removed, replaced, or fixed in some other
way before it can be included. Make sure to check that the full, terse,
and solution versions all compile when doing this. *)
...
(* NOTATION: IY --- Perhaps we can use ∈ for "only printing" for [\in]? For this proof in
   particular, it's a bit ugly to see [\in] floating around the context. *)
(* NOWISH: Ori: substitution_preserves_typing in Sub.v has now much simpler
     proof. Not sure how to update this proof here *)
...
Proof with eauto.
  ...
  - (* abs *)
    ...
    + (* x=y *)
      rewrite update_shadow in Htypt2. assumption.
    + (* x<>y *)
      apply IHt.
  ...
  - (* pair *)
    ...
    (* NOTATION: IY -- The spacing between [.] and [fst / snd] is a bit odd. *)
  - (* fst *)
