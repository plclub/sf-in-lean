...
Proof.
  ...
  - (* E_Asgn *) auto.
  - (* E_Seq *)
    assert (st' = st'0) as EQ1. auto. subst st'0. auto.
  (* E_IfTrue *)
  - (* b1 reduces to true *)
    auto.
  - (* b1 reduces to false (contradiction) *)
...
Proof.
  ...
  induction HT;
       ...
       try solve [inversion HE; subst; auto].
  - inverts* HE.
    applys* substitution_preserves_typing. inverts* HT1.
  (* /ADMIT *)
Admitted.
...
Proof.
  ...
  - right. destruct* IHHt1.
    destruct* IHHt2.
      inverts* H. inverts Ht1. inverts Ht1.
  - right. destruct* IHHt1.
    destruct* t1; solve_by_invert.
  (* /ADMIT *)
Admitted.
...
Proof.
  (* ADMIT *)
  introv [Hs Hnf]. rewrite nf_same_as_value in Hnf.
...
(* SOONER: Turn this into a real exercise! *)
...
Proof.
  (* ADMIT *)
  introv Hty. lets (S2&Hsub&Hty'): typing_inversion_abs Hty.
...
(* SOONER: this proof relies on [cases_if], however it is hidden
   in [UseTactics]... Below is a clean, up to date proof. *)
...
(** The lemma [substitution_preserves_typing] has already been used to
    ...
    fits on 22 lines (excluding comments). *)
...
Proof.
  (* ADMIT *)
  introv Htypt Htypv. gen S Gamma. induction t; introv Ht; simpl.
  - lets (T&EQ&Hsub): typing_inversion_var Ht.
    unfold update, t_update in EQ. cases_if*.
    + (* case [x=y] *)
      inverts EQ. apply* context_invariance.
      intros z Hz. lets* [T' HT']: free_in_context Hz. false.
  - lets*: typing_inversion_app Ht.
  - rename s into y, t into T1.
    lets (T&Hsub&HT): typing_inversion_abs Ht.
    apply* T_Sub. cases_if; apply T_Abs.
    + (* case [x=y] *)
      rewrite String.eqb_eq in *; subst.
      rewrite update_shadow in HT. assumption.
    + (* case [x<>y] *)
      rewrite String.eqb_neq in *; subst.
      apply IHt.
      rewrite update_permute. assumption.
      intros; subst; auto.
  - lets*: typing_inversion_true Ht.
  - lets*: typing_inversion_false Ht.
  - lets*: typing_inversion_if Ht.
  - lets*: typing_inversion_unit Ht.
  - lets*: typing_inversion_pair Ht.
  - lets*: typing_inversion_fst Ht.
  - lets*: typing_inversion_snd Ht.
  (* /ADMIT *)
Qed.
