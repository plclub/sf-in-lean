...
(* LATER: This chapter could use some exercises! *)
(* TERSE: HIDEFROMHTML *)
...
(** We saw in chapter \CHAP{MoreStlc} how records can be treated as
...
(* SOONER: We should try writing a custom induction tactic (by hand,
   using fix, following Andrew's example from earlier in the
   notes...) *)
...
(* INSTRUCTORS: Begin Definition of stlc_record *)
...
(* INSTRUCTORS: End Definition of stlc_record *)
...
(* LATER: Maybe if we put all the examples (here and elsewhere) in
   their own module, we could avoid polluting Rocq with these
   Notations.  Or we could put the Notations in a module and open it
   in the right places *)
...
Proof.
  (* ADMITTED *)
  intros C. destruct C as [T Hty].
  inversion Hty; subst.
  inversion H6.  Qed.
(* /ADMITTED *)

(* GRADE_THEOREM 1: typing_nonexample_2 *)
...
Proof.
  (* ADMITTED *)
  intros y C. destruct C as [T Hty].
  inversion Hty; subst.
  inversion H1; subst. clear Hty H1.
  inversion H3; subst.
  inversion H2; subst.
  inversion H8.  Qed.
(* /ADMITTED *)
...
(* Check the app rules, is this right? *)
...
(* NOTATION: NOWISH: Ori:
   AGAIN: why (x |-> U ; Gamma) and not x |-> U ; Gamma ? *)
