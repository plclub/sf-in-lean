...
(* INSTRUCTORS: This chapter gives an alternative treatment of Rocq
   automation that does not depend on \CHAP{Imp}.  It contains most of
   the automation material from that chapter, and most of the existing
   \CHAP{Auto} chapter except for material on [match].

   This chapter is suitable for a course that wants to move quickly on
   to VFA without doing any language metatheory at all. It's also
   suitable for readers who are studying LF to become familiar with
   Rocq, but do not plan on reading PLF and do not need to learn Imp.

   Since the nice examples from Imp are no longer available to give
   motivation for automation, we invent some functions on regular
   expressions (described in IndProp), and show how to automate these.
   It is possible these examples are less convincing, though. *)

(** FULL: So far, we've been doing all our proofs using just a small
    ...
    This chapter is an alternative to the combination of \CHAP{Imp}
    and \CHAP{Auto}, which cover roughly the same material about
    automation, but in the context of programming language metatheory.
    A deeper treatment of [auto] can be found in the \CHAPV2{UseAuto}
    chapter in _Programming Language Foundations_.  *)

(** TERSE: It's time to shorten our proofs through automation
    features:
...
(** TERSE: *** *)
(** FULL: As a simple illustration of the benefits of automation,
    let's consider another problem on regular expressions, which we
    formalized in \CHAP{IndProp}.  A given set of strings can be
...
(** TERSE: Here's a function that rewrites regular expressions into
...
(** FULL: The amount of repetition in that proof is annoying.  And if
...
(** FULL: There is no real reason to use [try] in completely manual
...
Proof. (* ADMITTED *)
  induction lst1 as [ | [ | h1] t1 IHl1]; intros;
    simpl; try rewrite IHl1; reflexivity.
Qed.
(* /ADMITTED *)
(* GRADE_THEOREM 1: nonzeros_app *)
...
Theorem add_assoc' : forall n m p : nat,
    n + (m + p) = (n + m) + p.
Proof. (* ADMITTED *)
  induction n; intros; [ idtac | simpl; rewrite IHn]; reflexivity.
Qed.
(* /ADMITTED *)
(* GRADE_THEOREM 1: add_assoc' *)
...
(** FULL: Note that there is no bar between [T1] and [..]. The bar can be supplied,
    though:
...
(** As an example, here is a lemma we proved in \CHAP{IndProp}: *)
...
(* LATER: APT: Should also talk about [fail]. And maybe mention [do] ?    *)
...
(* ADMITTED *)
  ...
  - (* MApp *)
    destruct re1;
      ...
      try inversion IH1; simpl; apply IH2.
  - (* MUnionL *)
    destruct re1;
      try (destruct re2;
  ...
  - (* MStarApp *)
    destruct re; try (apply star_app;
                      [(apply MStar1; apply IH1) | apply IH2]);
      try (inversion IH1; inversion IH2; apply MEmpty).
Qed.
(* /ADMITTED *)
(* GRADE_MANUAL 6: re_opt *)
(* QUIETSOLUTION *)
(** Another solution using [..] instead of [try]: *)
Lemma re_opt_match'_alt : forall T (re: reg_exp T) s,
  s =~ re -> s =~ re_opt re.
Proof.
intros T re s M.
  induction M
  ...
  - (* MUnionL *)
    destruct re1;
      [ inversion IH
      | destruct re2; [ apply IH | apply MUnionL; apply IH ..] ..].
  - (* MUnionR *)
    destruct re1;
      [ apply IH
...
(** FULL: So far we have been dependent on knowing the names of
    hypotheses.  For example, to prove the following simple theorem,
    we hardcode the name [HP]: *)

(** TERSE: Hardcoding of hypothesis names isn't great. *)
...
(** FULL: We took the trouble to invent a name for [HP], then we had
...
(** FULL: The [assumption] tactic is useful to streamline the proof
    above. It looks through the hypotheses and, if it finds the goal
    as one them, it uses that to finish the proof. *)

(* LATER: (MRC 3/22): of course [assumption] will do that if the
   hypothesis is convertible to the goal, but I don't think we have
   "covertible" established as vocabulary and here isn't the place to
   do it. *)

(** TERSE: Nameless is better. *)
...
(** FULL: Some might argue to the contrary that hypothesis names
...
(** TERSE: There's a tradeoff between ease of reading and ease of
...
(** FULL: Actually there are two forms of this tactic.
...
(** TERSE: The [constructor] tactic searches for an applicable
    constructor. *)
...
(** FULL: Warning: if more than one constructor can apply,
...
(* INSTRUCTORS: [lia] is also covered in vfa/Perms.v, so it could be
   skipped here or there. *)

(** FULL: The [lia] tactic implements a decision procedure for integer
...
(** TERSE: [lia] is a decision procedure for integer linear
    arithmetic. *)
...
(** FULL: The [lia] tactic makes use of facts about addition and
...
(** TERSE: We don't need to know what a function does to prove
    some equalities. *)
...
(** FULL: It is that congruence property that we're using when we
    [rewrite] in the proof above: if [a = b] then [f a = f b].  (The
    \CHAP{ProofObjects} chapter explores this idea further under the
...
(** FULL: The [congruence] tactic is able to work with constructors,
...
(** FULL: A _tautology_ is a logical formula that is always
...
(** TERSE: [intuition] implements a decision procedure for
...
(** FULL: In the previous example, neither [congruence] nor
...
(** TERSE: [intuition] takes an optional argument. *)
...
Qed.
(* /ADMITTED *)
(* GRADE_THEOREM 0.5: plus_id_exercise_from_basics *)
...
Qed.
(* /ADMITTED *)
(* GRADE_THEOREM 0.5: add_assoc_from_induction *)
...
Qed.
(* /ADMITTED *)
(* GRADE_THEOREM 0.5: S_injective_from_tactics *)
...
Qed.
(* /ADMITTED *)
(* GRADE_THEOREM 0.5: or_distributes_over_and_from_logic *)
...
(** TERSE: We can also extend hint databases for future uses:
...
(* SOONER: MRC 3/22: could use some easier [auto] exercises *)

(* EX3M (re_opt_match_auto) *)
...
(* QUIETSOLUTION *)
Create HintDb re.
Hint Constructors exp_match : re.
Hint Resolve star_app : re.
(* /QUIETSOLUTION *)
...
(* ADMITTED *)
  ...
  induction M
    as [| x'
        | s1 re1 s2 re2 Hmatch1 IH1 Hmatch2 IH2
        | s1 re1 re2 Hmatch IH | s2 re1 re2 Hmatch IH
        | re | s1 s2 re Hmatch1 IH1 Hmatch2 IH2]; simpl; auto with re.
  - (* MApp *)
    destruct re1;
      try (destruct re2; auto with re;
           try (inversion IH2; rewrite app_nil_r; auto));
      try inversion IH1; simpl; auto.
  - (* MUnionL *)
    destruct re1;
      try (destruct re2; auto with re);
      inversion Hmatch.
  - (* MUnionR *)
    destruct re1;
      ...
      inversion Hmatch.
  - (* MStar0 *)
    destruct re; auto with re.
  - (* MStarApp *)
    destruct re; auto with re;
      try (inversion IH1; inversion IH2; simpl; auto with re).
Qed.
(* /ADMITTED *)
(* GRADE_MANUAL 3: re_opt_match'' *)
(** [] *)

(* HIDE: MRC 3/22: Because there are now quite a few other exercises
   in this chapter, I made this one optional for my class -- most of
   whom had never done the pumping lemma exercises anyway. Feel free
   to restore the exercise to being required. *)
(* EX3AM? (pumping_redux) *)
(** Use [auto], [lia], and any other useful tactics from this chapter
    to shorten your proof (or the "official" solution proof) of the
    weak Pumping Lemma exercise from \CHAP{IndProp}. *)
...
(* ADMITTED *)
  ...
  induction Hmatch
    ...
    intros Hlen.
  - (* MApp *)
    simpl. rewrite app_length in *.
    assert (H : pumping_constant re1 <= length s1 \/
                pumping_constant re2 <= length s2) by lia.
    destruct H as [H | H].
  ...
  - (* MStarApp *)
    ...
    { induction s1 as [| h s1' IHs1]; auto.
      - right.
        assert (Hcases : length (h :: s1') < pumping_constant re \/ pumping_constant re <= length (h :: s1')).
        { apply PeanoNat.Nat.lt_ge_cases. }
        destruct Hcases as [Hlenlt | Hlengt]; auto.
        left. split; auto. unfold not; intros contra. inversion contra.
    }

    destruct Hs1re1 as [Hs1len0 | [[Hs1len Hs1re1] | Hs1re1]].
    + assert (Hs1nil : s1 = []).
      { destruct s1; auto. inversion Hs1len0. }
      subst; auto.
    + exists [], s1, s2. simpl in *.
      unfold not in *.
      repeat (split; intros; subst; auto using PeanoNat.Nat.lt_le_incl, napp_star).
    + destruct (IH1 Hs1re1) as [s11 [s12 [s13 [H1 [H2 Hnapp]]]]].
      exists s11, s12, (s13 ++ s2).
      repeat (split; subst; repeat rewrite <- app_assoc; auto).
      intros m.
      repeat rewrite -> app_assoc.
      apply MStarApp; auto.
      rewrite <- app_assoc; auto.
Qed. (* /ADMITTED *)
(* GRADE_MANUAL 3: pumping_redux *)
(** [] *)

(* EX3A? (pumping_redux_strong) *)
(** Use [auto], [lia], and any other useful tactics from this chapter
    to shorten your proof (or the "official" solution proof) of the
    stronger Pumping Lemma exercise from \CHAP{IndProp}. *)
...
Proof.
  ...
  - (* MApp *)
    assert (H : pumping_constant re1 <= length s1 \/
                pumping_constant re2 <= length s2) by (rewrite app_length in Hlen; lia).
    destruct H as [H | H].
    + destruct (IH1 H) as [s11 [s12 [s13 [H1 [H2 [Hlenp Hnapp]]]]]].
      rewrite H1.
      exists s11. exists s12. exists (s13 ++ s2).
      repeat rewrite <- app_assoc.
      repeat (split; auto).
    ...
    + (* now, either...  length s1 < pumping_constant re, or pumping_constant <= length s1 *)
      ...
      * destruct (IH2 H) as [s21 [s22 [s23 [H1 [H2 [Hlenp Hnapp]]]]]].
        rewrite H1.
        exists (s1 ++ s21). exists s22. exists s23.
        rewrite <- app_assoc; rewrite app_length in *; rewrite <- add_assoc.
        repeat (split;
                auto).
        lia.
        intros m.
...
Qed.
(* /ADMITTED *)
(* GRADE_MANUAL 3: pumping_redux_strong *)
(** [] *)
...
(* SOONER: The "Nat." prefix is required to make (only) the terse
   version build.  Not sure why. :-( *)
  - apply H1.
  - simpl in H2. rewrite mul_comm. apply H2.
Qed.

(** TERSE: [apply] fails if we leave out the [with], even though the
...
(** TERSE: *** *)
(** FULL: Several of the tactics that we've seen so far, including
    [exists], [constructor], and [auto], have [e...] variants.  For
    example, here's a proof using [eauto]: *)
(** TERSE: Again, using [eauto]. *)
...
(** FULL: The [eauto] tactic works just like [auto], except that it
...
(** TERSE: [eauto] is significantly slower than [auto]. Use only as
    needed. *)

(* SOONER: Perhaps another / better example? *)
(* SOONER: MRC 3/22: need some exercises *)

(* ####################################################### *)
(** * Ltac: The Tactic Language *)

(** FULL: Most of the tactics we have been using are implemented in
...
(** TERSE: New tactics can be defined in two ways:
...
(** TERSE: *** *)
(** FULL: Of course, that little tactic is not so useful. But it
...
(** For a more useful tactic, consider these three proofs from
    \CHAP{Basics}, and how structurally similar they all are: *)
...
(* EX1 (andb3_exchange) *)
(** Re-prove the following theorem from \CHAP{Basics}, using only
...
(* EX2 (andb_true_elim2) *)
(** The following theorem from \CHAP{Basics} can't be proved with
    [destructpf]. *)
...
Qed.
(* /ADMITTED *)
(* GRADE_THEOREM 1.5: andb_true_elim2' *)
...
Qed.
(* /ADMITTED *)
(* GRADE_THEOREM 0.5: andb3_exchange' *)
(** [] *)
...
(** FULL: At the point we [rewrite], we can't substitute away [t]: it
...
(** TERSE: How can we automate that pattern? Ltac has a [match goal]
    tactic. *)
...
(** FULL: The syntax is similar to a [match] in Gallina (Rocq's term
...
(** TERSE: That says: if the conclusion is [True] (regardless of the
...
(** TERSE: *** *)
(** FULL: The semantics of the tactic [match goal] have a big
...
(** TERSE: Ordinary [match] doesn't allow redundant branches. *)
...
(** FULL: But with [match goal], if the tactic for the branch fails,
    pattern matching continues with the next branch, until a branch
    succeeds, or all branches have failed. *)
(** TERSE: But [match goal] keeps trying until success. *)
...
(** FULL: The first branch was tried but failed, then the second
...
(** TERSE: *** *)
(** FULL: Next, let's try matching against hypotheses. We can bind a
    hypothesis name, as with [H] below, and use that name on the
    right-hand side of the branch. *)
(** TERSE: A match against a hypothesis: *)
...
(** TERSE: *** *)
(** FULL: We'll keep using [idtac] for awhile to observe the behavior
    ...
    all have failed). *)
(** TERSE: Matching will _backtrack_ and retry with other
...
(** FULL: That example prints "HQ" followed by "HP". Ltac first
...
(** TERSE: *** *)
(** FULL: So far we haven't been very demanding in how to match
    hypotheses. The _wildcard_ (aka _joker_) pattern we've used
    matches everything. We could be more specific by using
    _metavariables_: *)
(** TERSE: _Metavariables_ are pattern variables for the goal: *)
...
(** FULL: Note that this time, the only hypothesis printed by [idtac]
...
(** TERSE: That applies only [HP], because [HQ] doesn't match. *)

(** TERSE: *** *)
(** FULL: Now we have seen yet another difference between [match goal]
    ...
    that: *)
(** TERSE: With [match] against terms, pattern variables can't be
    repeated: *)
...
Proof. (* ADMITTED *)
  nor_intuition.
Qed. (* /ADMITTED *)
(* GRADE_MANUAL 6: nor_intuition *)
(** [] *)

(* HIDE *)
(* HIDE: MRC 3/22: Here is an encoding of NAND that makes it
   constructively equivalent to a disjunction. *)
Module DisjunctiveNand.

  Inductive nand (P Q : Prop) : Prop :=
  | stroke1 : ~P -> nand P Q
  | stroke2 : ~Q -> nand P Q.

  Theorem nand_or_not : forall (P Q : Prop),
      nand P Q <-> ~ P \/ ~Q.
  Proof.
    intros P Q. split.
    - intros Hnand. destruct Hnand as [HNP | HNQ].
      + left. assumption.
      + right. assumption.
    - intros Horn. destruct Horn as [HNP | HNQ].
      + apply stroke1. assumption.
      + apply stroke2. assumption.
Qed.
...
(* LATER: Could use exercises here on leveraging [match goal] with
   regular expression proofs. *)

(* HIDE: MRC 3/22: The following used to be in this file:
   ...
   But it was not used anywhere here, nor is it used in VFA anywhere
   outside of Binom and Color, where it doesn't even appear in the
   FULL version. I would nonetheless have preserved it if there were
   anywhere here it appeared to be useful. Yet, I found no such
   place. So I'm leaving it here, and perhaps it will be deleted in a
   later iteration.
   ...
   I guess that could be explained here in the sense of pruning the
   search space for [auto], but we go so little into that complication
   about [auto] that it doesn't seem worth it. The first time [clear]
   comes up in vfa/Perm.v (with "butterfly") the usage is, well, clear
   from context.  *)
