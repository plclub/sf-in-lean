...
(** TERSE: *** *)
(** TERSE: Observe how Rocq picks appropriate values for the
...
(** You can use [apply] with previously defined theorems, not
    ...
    previously-defined theorem about [rev] from \CHAP{Lists}.  Use
...
(** FULL: Actually, the name [y] in the [with] clause is not required,
...
(** TERSE: *** *)
(** FULL: Rocq also has a built-in tactic [transitivity] that
...
(** TERSE: [transitivity] is also available as a tactic. *)
...
(** FULL: Rocq's [assert] tactic, used above, adds the given hypothesis
...
(* HIDE *)
    ...
    (** Alternatively, if you just say [injection H] with no [as] clause,
        ...
        APT 21: oops, not so! So let's not even discuss [injection] with
        no [as]. *)
...
(** FULL: If we use [discriminate] on this hypothesis, Rocq confirms
...
*)
(* HIDE *)
Theorem quiz0 : forall x y : rgb, primary x = primary y -> y = x.
Proof. intros x y H. injection H as Hxy. symmetry. assumption. Qed.
(* /HIDE *)
(* /QUIZ *)
(* QUIZ *)
(** Suppose Rocq's proof state looks like
...
*)
(* HIDE *)
Theorem quiz1 : forall x y : bool, negb x = negb y -> y = x.
Proof. intros x y H. Fail injection H as Hxy. Abort.
(* /HIDE *)
(* /QUIZ *)
(* QUIZ *)
...
*)
(* HIDE *)
Theorem quiz2 : forall x y : nat, x + 1 = y + 1 -> y = x.
Proof. intros x y H. Fail injection H as Hxy. Abort.
(* /HIDE *)
(* /QUIZ *)
(* QUIZ *)
...
*)
(* HIDE *)
Theorem quiz3 : forall x y : nat, 1 + x = 1 + y -> y = x.
Proof. intros x y H. simpl in H. injection H as Hxy. symmetry. assumption. Qed.
(* /HIDE *)
(* /QUIZ *)
(* /TERSE *)
...
(** FULL: Indeed, there is also a tactic named `f_equal` that can
...
(** TERSE: Rocq also provides [f_equal] as a tactic. *)
...
(** FULL: Similarly, [apply L in H] matches some conditional statement
...
(* LATER: Maybe we should put one more good example to round out this
   section? *)
...
(** FULL: At this point, we need to show [S ((m' - n') + n') = S m'] from
    ...
    (** TERSE: We could use the [assert] tactic to prove [(m' - n') +
...
Proof.
  (* ADMITTED *)
  intros X l1 l2 x. induction l1 as [| y l1' IHl1'].
  - (* l1 = nil *)
...
(** FULL: We have seen many examples where [destruct] is used to
...
(** FULL: ... then we are stuck at this point because the context does
    ...
    [eqn:] qualifier does. *)
(** TERSE: *** *)
(** TERSE: Adding the [eqn:] qualifier saves this information so we
    can use it. *)
