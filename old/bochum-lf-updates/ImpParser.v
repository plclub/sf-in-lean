...
(* SOONER: The DO notation defined here should be unified with the
   notation defined in /plf/Typechecking.v -- that one is better, I
   think. *)
(* SOONER: This file should use concrete notations from Imp. *)
...
(* LATER: Couldn't we just import Imp if we did Require Export in Imp? *)
(* TERSE: HIDEFROMHTML *)
...
(* LATER: Old version (that expands to the wrong thing with multiple try blocks)
...
BETTER VERSION due to Remzi Yang:
*)
...
(* SOONER: Arguably, identifiers should be uppercase alpha, not lower! *)
...
(* NOTATION: SOONER: Fix concrete syntax here? *)
...
(* HIDE *)
Definition parse_ (str : string) : optionE (com * list token) :=
  let tokens := tokenize str in
...
(* SOONER: this gets typeset awkwardly (without line breaks) by coqdoc *)
Example eg1 : parse "
