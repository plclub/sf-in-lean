...
partial def walkBlock (width : Nat) (file : String) (b : Verso.Doc.Block Manual)
  ...
  | .other which contents =>
    ...
    if name == ``Block.gradeTheorem then
      let ⟨points, name⟩ := decodeGradeTheoremData which.data
      return appendBoth buf file (s!"attribute [autogradedProof {points}] {name}\n\n") -- TODO this is only needed in grading version
...
def walkOuter (width : Nat) (vol : String) (text : Part Manual) (buf : SaveBuffers) :
  ...
  for p in subParts do
    ...
    buf := appendBoth buf chapterFile s!"import AutograderLib\n" -- TODO this is only needed in grading version
...
private def emitSavedImpl (destSlug modPrefix variant : String)
  ...
  fun _mode _cfg _state text => do
    ...
    for (pre, part) in crossVol do
      ...
      buf := appendBoth buf chFile "import AutograderLib\n" -- TODO this is only needed in grading version
