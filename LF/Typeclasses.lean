...
Intuitively, a total map over an element type `β` just contains a function `toFun` from a key of type `α` to a value of type `β`.
...
It lets us prove `m₁ = m₂` from `m₁.toFun = m₂.toFun` or vice versa.
