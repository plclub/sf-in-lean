...
So far we have discussed what is sometimes called _parametric polymorphism_, where in Lean we can
...
without any further information about the details of the type `α`. In this we we are able to work
with type like {InlineLean.lean}`List α`, and can write function like {name}`List.reverse` or
{name}`List.length` that only operate on the structure of a list independently of its contents,
along with corresponding proofs like {name}`List.length_reverse` that are similarly structural in
nature.

This is somewhat limiting though, as this means that there is no way that we can inspect or
non-trivially use any particular `a : α`. What we might want in some situations rather than `α`
being completely generic is to partially specify its behavior. In Lean, this happens through a form
of "ad hoc polymorphism" called *typeclasses*. This concept originating in the Haskell programming
language, and is analogous to concepts such as traits in Rust or interfaces in Java that you may
already be familiar with.

# A First Example: Inhabited Types

As an example, suppose that I wanted a way to specify that a type has at least one inhabitant, that
is that it is not empty. In Lean, we could express this as the typeclass
...
which explicitly carries around an element `one : α` as a witness. After defining this typeclass,
we can now provide *instances* that show it is satisfied for a particular type. As an example,
we could declare an instance
...
When in doubt, you can also use the option `pp.all` like:
...
In most languages that support typeclasses this is the extent of what a typeclass can enforce,
carrying the data of a type, but relying on the author to enforce any invariants. As an example,
suppose that we wanted to express the idea that a type has at least two elements. A first attempt
might be
...
:::dev
David's example repo had this `-- SOLUTION` syntax for when we want the student to write definitions
in an exercise, but it seems to not be working with the Verso build -- CGH
:::
...
:::dev
In a couple of places I want to reference a previous chapter that is not yet in Verso format,
so I leave a <LINK> placeholder. At the moment, this is written as if after the (Rocq) `IndProp`
chapter.
:::

In previous chapters we have seen how polymorphism in Lean allows us to declare a type variable


that work with functions like {name}`List.reverse`. This variety of polymorphism
is called *parametric* because it works identically for any given type, so that
whether we are working with {InlineLean.lean}`List.reverse (α := Nat)` or with
{InlineLean.lean}`List.reverse (α := Int)`, proofs we have constructed like
{name}`List.length_reverse` are independent of the type used.

There are some functions we can write however where it is unclear how we would write a polymorphic
equivalent. As an example, consider the follow function for lists of natural numbers:
...
that mentions a {deftech}_typeclass_ {name}`BEq`. Perhaps a better question is what exactly
the `==` notation meant in our original {name}`List.elem_nat`. Let's look at a usage of this
notation with natural numbers, but turn off notation:
...
expected `Nat.beq`. This process of

What is happening behind the scenes when we use the `a == b` notation is that Lean sees that
this
